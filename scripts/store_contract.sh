...
CHAINID="euphoria-2"
CONTRACT_DIR="artifacts/instantiate2_factory.wasm"
SLEEP_TIME="15s"

RES=$(aurad tx wasm store "$CONTRACT_DIR" --from "$ACCOUNT" -y --output json --chain-id "$CHAINID" --node "$NODE" --gas 3000000 --fees 35000ueaura -y --output json)
echo $RES
